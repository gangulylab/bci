...
while chk>1e-10 || chk1>1e-10 
    ...
    temp_norm1 = norm(K(:));  
    P = A*P*A' + W;
    P(1:3,:) = zeros(3,size(P,2)); % no position measurement error due to visual feedback
    P(:,1:3) = zeros(size(P,1),3); % no position measurement error due to visual feedback
    P(:,end)=0; % no error in estimate of mean neural activity
    P(end,:)=0;% no error in estimate of mean neural activity
    K = P*C'*pinv(C*P*C' + Q);
    P = P - K*C*P;
