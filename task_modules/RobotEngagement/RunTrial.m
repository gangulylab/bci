...
Cursor.State(1:3) = Params.StartPos(Data.TargetID,:);
...
Params.TrialInputOrder  = Params.InputOrder*Params.TargetID;
Params.index = 0;
...
if ~Data.ErrorID
    ...
    while ~done
        ...
        if (tim-Cursor.LastPredictTime) > 1/Params.ScreenRefreshRate
            ...
            correctInput            = Params.TrialInputOrder(trajIndex);
            goState                 = correctInput > 0;
            Data.GoState(1,end+1)   = goState;
            
            if mod(trajIndex, Params.GoBins+Params.StopBins) == Params.GoBins
                fwrite(Params.udp, [0, 20, 0]);
            elseif mod(trajIndex, Params.GoBins+Params.StopBins) == 1
                fwrite(Params.udp, [0, 20, 1]);
            end
            ...
            U = zeros(3,1);
            U(1) = int8(input == 1) - int8(input == 3);
            U(2) = int8(input == 2) - int8(input == 4);
            U(3) = int8(input == 5) - int8(input == 6);
            ...
            if Params.ShowUserDecode == 1
                send = ClickToSend;
            else
                send = correctInput;
            end
            
            fwrite(Params.udp, [4, xa,xb,xc,ya,yb,yc, za,zb,zc, send]);
