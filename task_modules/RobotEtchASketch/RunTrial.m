...
%             TargetID = InTargetRobot3D(Cursor,Params.ReachTargetPositions,Params.RobotTargetRadius, Params.RobotTargetDim, Data.TargetID);
            ...
            TargetID = InTargetRobot3D2(Cursor,ReachTargetPos,Params.RobotTargetRadius, Params.RobotTargetDim, Data.TargetID);

            if TargetID && ~targetdone
            ...
            correctU = Params.CorrectDecode{Data.TargetID}(stepInd);
            Data.CorrectDecode(1,end+1) = correctU;

            if RunningMode_ClickDec == correctU
                stepInd = stepInd + 1;
                Cursor.State = [[Params.PathPoints{Data.TargetID}(stepInd,:)]';0;0;0];
                if stepInd == max(find(Params.CorrectDecode{Params.TargetID} == 7))+1
            ...
            Data.CursorState(:,end+1) = Cursor.State';
            Data.StepInd = stepInd;
                       
            Cursor.TaskState = 3;
            Data.TaskState(1,end+1)=Cursor.TaskState;
             
        
        if stepInd >= length(Params.CorrectDecode{Data.TargetID}) 
            done = 1;
            fprintf('SUCCESS')
            Data.SelectedTargetID = 1; %success
        end 
