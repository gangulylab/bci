...
%% Hand
Params.TargetNames = {'Thumb', 'Index', 'Middle', 'Ring', 'Pinky', 'Power Grasp',...    
...
Params.NumActionsPerTrial   = 5;
Params.Actions              = [1:8,10,12];
numRepeat   = ceil(Params.NumTrialsPerBlock*Params.NumActionsPerTrial/length(Params.Actions));
targetOrder = [];
for i = 1:numRepeat
targetOrder = [targetOrder, Params.Actions(randperm(length(Params.Actions)))];
end
% targetOrder = repmat(Params.Actions',numRepeat, 1)';  
targetOrder = reshape(targetOrder', Params.NumActionsPerTrial,[])';  % shuffle and reshape
Params.BlockTargetOrder = num2cell(targetOrder,2);

Params.handVis = 1;
Params.ActionBins = [6:10];  % (half cycle)
% Params.ActionDurationDeviation = 3; % seconds
Params.TransitionBins = [5,6,7];
Params.PauseBins = 2;
% Params.TransitionDurationDeviation = 3;
