...
if Data.TargetID  == 1
    fwrite(Params.udp, [0, 24, 1]);  
    Cursor.Rot(1) = 0;
    goalAng = pi/2*100;
elseif Data.TargetID == 2
     fwrite(Params.udp, [0, 24, 2]);
     Cursor.Rot(1) = pi/2*100;
     goalAng = 0;
end

Params.GoalAng = goalAng;
...
if ~Data.ErrorID,
    ...
    while ~done
        ...
        if (tim-Cursor.LastPredictTime) > 1/Params.ScreenRefreshRate,
            ...
            if abs(Cursor.Rot(1) - goalAng) < 17.45
                inTarget = 1;
            ...
            if TaskFlag==1 % imagined movements
                ...
                % Rotation x only
                Ur(1) =  int8(RunningMode_ClickDec == 8) - int8(RunningMode_ClickDec == 9);   
                Cursor.RotV(1) = Params.rA(1)*Cursor.RotV(1) + Params.rB*Ur;
                Cursor.Rot(1) = Cursor.Rot(1) + Cursor.RotV(1);
                ...
                Data.Rot(:,end+1) = Cursor.Rot;
                Data.RotV(:,end+1) = Cursor.RotV;
            ...
            [xa,xb,xc] = doubleToUDP(Cursor.RotV(1));
            
            fwrite(Params.udp, [16, xa,xb,xc,ya,yb,yc, za,zb,zc, ClickToSend]);
