% Reanimates data from Robot3D (Robot) task
...
for dateInd = 1:numel(dates)    % iterate dates
    date        = dates{dateInd};
    taskDir     = [dataDir,'/bravo1/', dates{dateInd},'/GangulyServer/', dates{dateInd}, '/Robot/'];
    tmp         = dir(taskDir);
    blockDirs   = {tmp.name};
